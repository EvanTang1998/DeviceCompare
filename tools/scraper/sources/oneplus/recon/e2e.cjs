// 端到端验证：一加机型在页面上的可用性（下拉列表 / 产品图配对 / 色环切换）
// 前置：本机 dev server 已起在 5173（在项目根跑 npm run dev）
//
// 用法：
//   node sources/oneplus/recon/e2e.cjs                  # 逐台检查下拉列表里的全部一加机型
//   node sources/oneplus/recon/e2e.cjs "一加 Ace 6T"    # 只查指定机型（可给多个）
//
// 重点不是「页面不报错」，而是**图有没有配对到对的机型**：
// 从 src/data/devices/*.json 读出「机型名 → 机型 id」，再比对页面上那张图的文件名是否以该 id 开头。
// 配错色、抓错机型这类问题只有这样才抓得住。
...
for (const f of fs.readdirSync(devicesDir)) {
  ...
  if (d.brand === "一加") idByName.set(d.name, f.replace(/\.json$/, ""));
}
...
(async () => {
  ...
  // 触发按钮的文字带一个下拉箭头（「一加 15T▾」），图片 src 在 dev server 下还带 ?t= 时间戳，
  // 两处都要先洗掉再比对
  ...
  // 1. 品牌切到「一加」，列出下拉里的全部一加机型
  await pg.selectOption(col + " >> .picker select[aria-label=品牌]", { label: "一加" });
  await pg.waitForTimeout(300);
  await pg.click(modelBtn);
  await pg.waitForTimeout(400);
  const allItems = await pg.locator(".model-menu .model-item").allTextContents();
  ...
  console.log(`下拉里的一加机型：${allItems.length} 台`);
  console.log(`本次检查：${targets.length} 台\n`);

  // 2. 逐台选中，核对产品图文件名与机型 id 是否对得上、色环数量
  for (const name of targets) {
    ...
    console.log(`${imgOk ? "✓" : "✗"} ${picked.padEnd(16)} 图=${String(img).padEnd(38)} 色环=${swatches}${switched ? ` → ${switched}` : ""}`);
  }
  await pg.screenshot({ path: "/tmp/oneplus-e2e.png" });
  console.log(`\n截图：/tmp/oneplus-e2e.png`);
